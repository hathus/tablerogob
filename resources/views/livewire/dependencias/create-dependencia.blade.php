...
<form wire:submit.prevent="editCompromise"
    ...
    {{-- Campo: Número de dependencia --}}
    <div class="relative"> {{-- x-show y x-transition eliminados --}}
        <label for="compromiso_numero"
               class="block mb-1 text-sm font-medium text-gray-600 dark:text-gray-300 uppercase">Número de la Dependencia</label>
        <input id="compromiso_numero" type="number" min="1"
    ...
    <div class="relative"> {{-- x-show y x-transition eliminados --}}
        <label for="compromiso_nombre"
               class="block mb-1 text-sm font-medium text-gray-600 dark:text-gray-300 uppercase"> Nombre de la Dependencia</label>
        <input id="compromiso_nombre" type="text"
    ...
    {{-- Campo: Siglas de la Dependencia --}}
    <div class="relative"> {{-- x-show y x-transition eliminados --}}
        <label for="compromiso_siglas"
               class="block mb-1 text-sm font-medium text-gray-600 dark:text-gray-300 uppercase"> Siglas de la Dependencia</label>
        <input id="compromiso_siglas" type="text"
               wire:model.defer="compromiso_siglas"
               placeholder="Siglas de la Dependencia"
               class="peer w-full pl-4 pr-10 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white/70 dark:bg-gray-800/60 backdrop-blur-md text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-sky-500 focus:outline-none transition duration-200 placeholder-gray-400 dark:placeholder-gray-500"/>
        <div class="absolute right-3 top-9">
            @error('compromiso_siglas')
            <svg class="w-5 h-5 text-sky-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path d="M6 18L18 6M6 6l12 12" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            @else
                @if (!empty($compromiso_siglas))
                    <svg class="w-5 h-5 text-sky-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path d="M5 13l4 4L19 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                @endif
    ...
    {{-- Campo: Titular de la Dependencia --}}
    <div class="relative"> {{-- x-show y x-transition eliminados --}}
        <label for="compromiso_titular"
               class="block mb-1 text-sm font-medium text-gray-600 dark:text-gray-300 uppercase"> Titular de la Dependencia</label>
        <input id="compromiso_titular" type="text"
               wire:model.defer="compromiso_titular"
               placeholder="Titular de la Dependencia"
               class="peer w-full pl-4 pr-10 py-3 rounded-xl border border-gray-300 dark:border-gray-600 bg-white/70 dark:bg-gray-800/60 backdrop-blur-md text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-sky-500 focus:outline-none transition duration-200 placeholder-gray-400 dark:placeholder-gray-500"/>
        <div class="absolute right-3 top-9">
            @error('compromiso_titular')
            <svg class="w-5 h-5 text-sky-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path d="M6 18L18 6M6 6l12 12" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
            @else
                @if (!empty($compromiso_titular))
                    <svg class="w-5 h-5 text-sky-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path d="M5 13l4 4L19 7" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                @endif
    ...
    {{-- Campo: Dependencia de sector (Toggle Switch) --}}
    <div x-data="{ dependencia_sector: @entangle('dependencia_sector') }" {{-- x-show y x-transition eliminados aquí --}}
         class="relative flex items-center justify-between">
        <label for="dependencia_sector" class="text-sm font-medium text-gray-600 dark:text-gray-300 uppercase">Dependencia de sector</label>
        <label for="dependencia_sector" class="flex items-center cursor-pointer">
            <div class="relative">
                <input type="checkbox" id="dependencia_sector" class="sr-only" wire:model.defer="dependencia_sector">
                <div class="block bg-gray-600 w-14 h-8 rounded-full"></div>
                <div class="dot absolute left-1 top-1 bg-white w-6 h-6 rounded-full transition"></div>
            </div>
            <div class="ml-3 text-gray-700 dark:text-gray-300 font-medium">
                <span x-text="dependencia_sector ?"></span> {{-- Agregué 'Sí' y 'No' --}}
            </div>
        </label>
        <style>
...
<style>
    ...
    <div>
        <button type="submit"
            ...
            <span x-show="!loading">CREAR DEPENDENCIA</span>
        </button>
    </div>
